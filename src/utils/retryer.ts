...
const RETRIES = 7;
export async function retryer(fetcher: fetcherType, variables: { login: string }, retries = 0) {
    if (!RETRIES) {
    ...
    try {
        ...
        let response = await fetcher(variables, import.meta.env.GITHUB_API_TOKEN as string);
        ...
        if (err instanceof AxiosError) {
            ...
            if (isBadCredential || isAccountSuspended) {
                ...
                return err.response as AxiosResponse<{ data: Data } | { errors: Errors }, any> | undefined;
            }
        }
